...
# 1. kubeadm init
# -----------------------------------
echo "[1/6] kubeadm init baslatiliyor..."
kubeadm init \
...
# -----------------------------------
KUBECONF="$REAL_HOME/.kube/config"

echo "[3/6] Control-plane taint kaldiriliyor (master'a da pod schedule edilebilsin)..."
kubectl --kubeconfig="$KUBECONF" taint nodes "$MASTER_HOSTNAME" node-role.kubernetes.io/control-plane:NoSchedule- || true
...
kubectl --kubeconfig="$KUBECONF" wait --namespace kube-system \
...
kubectl --kubeconfig="$KUBECONF" wait --namespace metallb-system \
  --for=condition=ready pod \
  --selector=app=metallb \
  --timeout=120s

# MetalLB IPAddressPool - config'den generate et
kubectl --kubeconfig="$KUBECONF" apply -f - <<EOF
...
# -----------------------------------
echo ""
echo "[6/6] Worker join komutu olusturuyor..."
